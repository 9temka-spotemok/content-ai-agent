...
def create_openai_client(api_key: str = None):
    ...
    # Используем переданный ключ или ключ из конфига
...
def render_sidebar():
    ...
    # Статус API ключа
    api_status = "Подключен" if Config.OPENAI_API_KEY else "Не подключен"
    if st.session_state.theme == "dark":
        api_color = "#ffffff" if Config.OPENAI_API_KEY else "#888888"
    else:
        api_color = "#000000" if Config.OPENAI_API_KEY else "#666666"
